...
from algorithm.topk.client import CLIENT
...
class SERVER:
    def __init__(self, config):
        self.config = config
        assert self.config.errorType in ["local", "none"]
    ...
    def federate(self):
        ...
        for i in tqdm(range(self.config.numRounds)):
            ...
            weight_update = torch.zeros_like(self.server_weights)
            for (trainSamplesNum, update) in self.updates:
                weight_update += trainSamplesNum / sumOfSampleNum * update.cpu()
            param_vec = self.server_weights + weight_update
